                ...
                {loading ? (
                    ...
                    <div className="space-y-12 animate-in fade-in duration-1000">
                        ...
                        <div className="space-y-8">
                            {!searchQuery && ( <div className="flex items-center gap-3 px-1"><div className={`w-1.5 h-1.5 rounded-full ${mediaFilter === 'AUDIO' ? 'bg-emerald-500' : 'bg-indigo-500'}`}></div><h2 className="text-[11px] font-black text-white uppercase tracking-[0.3em] flex items-center gap-4">{selectedCategory !== 'TODOS' ? `Filtrando por: ${selectedCategory}` : (parentFolderName ? `Contenido en ${parentFolderName}` : 'Novedades')}{mediaFilter !== 'ALL' && <span className="text-slate-500 text-[9px] lowercase italic">({mediaFilter.toLowerCase()}s)</span>}{(userSortOrder || appliedSortOrder) && <span className={`text-[9px] lowercase border px-2 py-0.5 rounded-full ${userSortOrder ? 'text-indigo-400 border-indigo-500/30' : 'text-slate-500 border-slate-700'}`}>orden: {sortOptions.find(o => o.id === (userSortOrder || appliedSortOrder))?.label.toLowerCase() || appliedSortOrder.toLowerCase()}</span>}<span className="w-12 h-px bg-white/10"></span></h2></div> )}
                            ...
                            {videos.length > 0 ? ( <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-12">{videos.map(v => ( <VideoCard key={v.id} video={v} isUnlocked={isAdmin || user?.id === v.creatorId} isWatched={watchedIds.includes(v.id)} context={{ query: searchQuery, category: selectedCategory, folder: currentFolder, page: page, sortOrder: userSortOrder || appliedSortOrder }} /> ))}</div> ) : (folders.length === 0 && !loading) && ( <div className="text-center py-40 opacity-20 flex flex-col items-center gap-4"><Folder size={80} /><p className="font-black uppercase tracking-widest">Sin contenido disponible</p></div> )}
                        </div>
